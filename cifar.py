...
class dataset_wrapper(Dataset):
    def __init__(self, data_dir, dataset, datatype, label, label_root=None, return_meta=False):
        ...
        meta = {
            'iwildcam': {
                'mean': (0.3849, 0.3890, 0.3611),
                'std': (0.2642, 0.2640, 0.2634),
                'num_class': 182
            },
            'camelyon17': {
                'mean': (0.7456, 0.5908, 0.7229),
                'std': (0.1715, 0.2066, 0.1660),
                'num_class': 2
            }
        }
        
        self.num_class = meta[dataset]['num_class']
            ...
            transform = transforms.Compose(
                [
                    ...
                    transforms.Normalize(
                        meta[dataset]['mean'], meta[dataset]['std']
                    ),
                ]
            )
        else:
        ...
        self.dataset = get_dataset(dataset=dataset, download=True, root_dir=data_dir).get_subset(
            datatype, transform=transform
        )
        
        if "category" not in label:
            assert label_root is not None
            self.mels = np.load(label_root)
        else:
            self.mels = None
            
    def __getitem__(self, index):
        img, target, meta = self.dataset.__getitem__(index)
        if self.return_meta:
            if "category" in self.label:
                return (img, target, meta)
            else:
                return (img, self.mels[target], (target, meta))
        else:
            if "category" in self.label:
                return (img, target)
